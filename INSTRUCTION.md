...
データの出所を明記したい場合は、`config.yml` に `source`（出典名）・`sourceUrl`（出典 URL、任意）を追記して下さい。両方とも任意項目で、未設定でも動作に影響しません。

```yaml
category: 防災関連施設_警察署_交番
name: 防災関連施設（警察署・交番）
dataType: location
source: 静岡県警察
sourceUrl: https://www.police.pref.shizuoka.jp/
```
